...
import { getEnv, getLocalDirPath, IMAGE_QUEUE_NAME, redisConnection } from "@digital-asset-manager/shared";
...
const processor =
  getEnv().nodeEnv !== "production"
    ? async (job: Job) => {
        const processorModule = await import("./image.processor.js");
        return processorModule.default(job);
      }
    : imageProcessorUrl;

const imageWorker = new Worker(IMAGE_QUEUE_NAME, processor, {
